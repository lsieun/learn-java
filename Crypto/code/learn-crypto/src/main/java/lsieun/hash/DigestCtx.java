...
import lsieun.hash.sha1.SHA1Const;
import lsieun.hash.sha1.SHA1Utils;
import lsieun.hash.sha256.SHA256Const;
...
public class DigestCtx {
    ...
    public final HashBlockAlgorithm block_algorithm;
    public final HashFinalizeAlgorithm finalize_algorithm;
    public final HashEncodeAlgorithm encode_algorithm;

    // Temporary storage
    public final byte[] input_block = new byte[HashConst.DIGEST_BLOCK_SIZE];
    public int block_len;

    public DigestCtx(int[] hash, HashBlockAlgorithm block_algorithm, HashFinalizeAlgorithm finalize_algorithm, HashEncodeAlgorithm encode_algorithm) {
        this.hash = hash;
