...
/*      System.out.println("out[" + i + "] = \t" +
    ...
    private void Transform(MD5State state, byte buffer[], int shift) {
        ...
        state.state[2] += c;
        state.state[3] += d;
        System.out.println(String.format("a=%s, b=%s, c=%s, d=%s", state.state[0],state.state[1],state.state[2],state.state[3]));
    }
...
/*    System.out.print("Offset = " + offset + "\tLength = " + length + "\t");
    ...
    public void hashUpdate(String in) {
        Update(in);
    }

    public void hashUpdate(byte[] in) {
        Update(in);
    }

    public synchronized byte[] hashFinal() {
        return Final();
    }
    ...
    public static void main(String[] args) {
        if (args.length == 0) {
            System.out.println("usage: java MD5 <message>");
            System.exit(-1);
        }
