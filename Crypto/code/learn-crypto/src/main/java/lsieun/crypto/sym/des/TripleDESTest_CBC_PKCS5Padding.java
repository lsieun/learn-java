...
public class TripleDESTest_CBC_PKCS5Padding {
    ...
    public static void main(String[] args) {
        ...
        fm.format(FORMAT, "original bytes", ByteUtils.toHex(input));
        fm.format(FORMAT, "encrypted bytes", ByteUtils.toHex(encrypted_bytes));
        fm.format(FORMAT, "decrypted bytes", ByteUtils.toHex(decrypted_bytes));
        fm.format(FORMAT, "pad removed bytes", ByteUtils.toHex(pad_removed_bytes));
