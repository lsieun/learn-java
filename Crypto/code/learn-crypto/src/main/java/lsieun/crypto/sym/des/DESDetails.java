...
public class DESDetails {
    ...
    public static byte[] des_block_operate(byte[] input_64_bit_bytes, byte[] key_64_bit_bytes, CipherType type) {
        ...
        fm.format(FORMAT, "input(64-bit)", ByteUtils.toHex(input_64_bit_bytes));

        // Initial permutation
        byte[] content_64_bit_bytes = DESUtils.permute(input_64_bit_bytes, DESConst.ip_table);
        fm.format(FORMAT, "Initial Permutation(64-bit)", ByteUtils.toHex(content_64_bit_bytes));

        // Key schedule computation
        byte[] key_56_bit_bytes = DESUtils.permute(key_64_bit_bytes, DESConst.pc1_table);
        fm.format(FORMAT, "key(64-bit)", ByteUtils.toHex(key_64_bit_bytes));
        fm.format(FORMAT, "PC-1 key(56-bit)", ByteUtils.toHex(key_56_bit_bytes));
        fm.format("%n");
        ...
        for (int i = 1; i <= 16; i++) {
            ...
            fm.format(FORMAT, "subkey(48-bit)", ByteUtils.toHex(current_48_bit_sub_key_bytes));
            ...
            fm.format(FORMAT, "content(64-bit)", ByteUtils.toHex(content_64_bit_bytes));
            fm.format(FORMAT, "left content(32-bit)", ByteUtils.toHex(left_32_bit_bytes));
            fm.format(FORMAT, "right content(32-bit)", "        " + ByteUtils.toHex(right_32_bit_bytes));

            byte[] expansion_48_bit_bytes = DESUtils.permute(right_32_bit_bytes, DESConst.expansion_table);
            fm.format(FORMAT, "right expansion(48-bit)", "        " + ByteUtils.toHex(expansion_48_bit_bytes));
            byte[] xor_48_bit_bytes = DESUtils.xor(expansion_48_bit_bytes, current_48_bit_sub_key_bytes, 6);
            fm.format(FORMAT, "right xor subkey(48-bit)", "        " + ByteUtils.toHex(xor_48_bit_bytes));
            byte[] substitution_32_bit_bytes = DESUtils.get_substitution(xor_48_bit_bytes);
            fm.format(FORMAT, "substitution(32-bit)", "        " + ByteUtils.toHex(substitution_32_bit_bytes));
            byte[] permutation_32_bit_bytes = DESUtils.permute(substitution_32_bit_bytes, DESConst.p_table);
            fm.format(FORMAT, "permutation(32-bit)", "        " + ByteUtils.toHex(substitution_32_bit_bytes));
            byte[] xor_32_bit_bytes = DESUtils.xor(permutation_32_bit_bytes, left_32_bit_bytes, 4);
            fm.format(FORMAT, "new right(32-bit)", "        " + ByteUtils.toHex(xor_32_bit_bytes));
            ...
            fm.format(FORMAT, "new content(64-bit)", ByteUtils.toHex(content_64_bit_bytes));
            fm.format("%n");
        }
        ...
        fm.format(FORMAT, "the last swap(64-bit)", ByteUtils.toHex(swap_bytes));

        // (3) Final permutation (undo initial permutation)
        byte[] encrypted_64_bit_bytes = DESUtils.permute(swap_bytes, DESConst.fp_table);
        fm.format(FORMAT, "Final Permutation(64-bit)", ByteUtils.toHex(encrypted_64_bit_bytes));
